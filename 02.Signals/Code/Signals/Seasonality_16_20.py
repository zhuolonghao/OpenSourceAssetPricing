...
def Seasonality_16_20(base=base, keep_all=False):
    df = base.copy()
    df['ret_shift'] = df.groupby('ticker')['close'].pct_change(1).shift(181)
    ...
    df['ret_shift2'] = df.groupby('ticker')['close'].pct_change(1).shift(192)
    df['ret_shift3'] = df.groupby('ticker')['close'].pct_change(1).shift(204)
    df['ret_shift4'] = df.groupby('ticker')['close'].pct_change(1).shift(216)
    df['ret_shift5'] = df.groupby('ticker')['close'].pct_change(1).shift(228)
    df['ret_shift6'] = df.groupby('ticker')['close'].pct_change(1).shift(240)
    ...
    df['Season_1620'] = df.groupby('date_ym', group_keys=False)['on_season'].apply(lambda x: _bin(x, 10)).astype(str)
    df['OffSeason_1620'] = df.groupby('date_ym', group_keys=False)['off_season'].apply(lambda x: _bin(x, 10)).astype(str)
