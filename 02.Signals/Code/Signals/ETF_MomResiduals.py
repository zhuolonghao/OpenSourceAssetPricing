...
def MomResiduals(base=base, ff=ff, keep_all=False):
    ...
    df['MomResiduals6m'] = df.groupby('date_ym', group_keys=False)['MomResid6m'].apply(lambda x: _bin(x, 5)).astype(str)
    df['MomResiduals12m'] = df.groupby('date_ym', group_keys=False)['MomResid12m'].apply(lambda x: _bin(x, 5)).astype(str)
