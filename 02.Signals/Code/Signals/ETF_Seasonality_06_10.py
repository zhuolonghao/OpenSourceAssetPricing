...
def Seasonality_06_10(base=base, keep_all=False):
    ...
    df['Season_0610'] = df.groupby('date_ym', group_keys=False)['on_season'].apply(lambda x: _bin(x, 5)).astype(str)
    df['OffSeason_0610'] = df.groupby('date_ym', group_keys=False)['off_season'].apply(lambda x: _bin(x, 5)).astype(str)

    if keep_all:
        return df
    else:
        print('Completed: Seasonality_06_10')
