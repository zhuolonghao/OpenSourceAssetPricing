...
def Seasonality_02_05(base=base, keep_all=False):
    ...
    df['ret_shift2'] = df.groupby('ticker')['close'].pct_change(1).shift(24)
    df['ret_shift3'] = df.groupby('ticker')['close'].pct_change(1).shift(36)
    df['ret_shift4'] = df.groupby('ticker')['close'].pct_change(1).shift(48)
    df['ret_shift5'] = df.groupby('ticker')['close'].pct_change(1).shift(60)
