...
def Seasonality_11_15(base=base, keep_all=False):
    ...
    df['Season_1115'] = df.groupby('date_ym', group_keys=False)['on_season'].apply(lambda x: _bin(x, 5)).astype(str)
    df['OffSeason_1115'] = df.groupby('date_ym', group_keys=False)['off_season'].apply(lambda x: _bin(x, 5)).astype(str)


    if keep_all:
        return df
    else:
        print('Completed: Seasonality_11_15')
