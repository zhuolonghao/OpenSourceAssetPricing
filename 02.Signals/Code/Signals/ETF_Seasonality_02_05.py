...
def Seasonality_02_05(base=base, keep_all=False):
    ...
    df['Season_0205'] = df.groupby('date_ym', group_keys=False)['on_season'].apply(lambda x: _bin(x, 5)).astype(str)
    df['OffSeason_0205'] = df.groupby('date_ym', group_keys=False)['off_season'].apply(lambda x: _bin(x, 5)).astype(str)
